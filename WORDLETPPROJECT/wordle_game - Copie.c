...
void saveFeedbackToFile(const char *guess, const int *feedback, int attempt);
...
int main() {
    ...
    do {
        ...
        for (attempts = 0; attempts < MAX_ATTEMPTS && !won; attempts++) {
            ...
            /* Save feedback to file for solver */
            saveFeedbackToFile(guess, feedbacks[attempts], attempts + 1);
...
void saveFeedbackToFile(const char *guess, const int *feedback, int attempt) {
    ...
    fprintf(f, "%s\n", guess);
    for (int i = 0; i < WORD_LENGTH; i++) {
        if (feedback[i] == CORRECT_POS) fprintf(f, "G");
        else if (feedback[i] == WRONG_POS) fprintf(f, "Y");
        else fprintf(f, ".");
    }
    fprintf(f, "\n");
    fclose(f);
